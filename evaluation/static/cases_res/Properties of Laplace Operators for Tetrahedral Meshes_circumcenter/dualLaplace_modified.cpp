...
struct myExpressionResultType {
    ...
    myExpressionResultType(const Eigen::Matrix<double, 3, 1> & l,
               const Eigen::Matrix<double, 3, 1> & ba,
               const Eigen::Matrix<double, 3, 1> & cc)
    : l(l),
    ba(ba),
    cc(cc)
    {}
...
{
    Eigen::Matrix<double, 3, 1> l_0;
    l_0 << pow((b - c).lpNorm<2>(), 2), pow((a - c).lpNorm<2>(), 2), pow((a - b).lpNorm<2>(), 2);
    Eigen::Matrix<double, 3, 1> l = l_0;

    Eigen::Matrix<double, 3, 1> ba_0;
    ba_0 << l[1-1] * (l[2-1] + l[3-1] - l[1-1]), l[2-1] * (l[3-1] + l[1-1] - l[2-1]), l[3-1] * (l[1-1] + l[2-1] - l[3-1]);
    Eigen::Matrix<double, 3, 1> ba = ba_0;

    Eigen::Matrix<double, 3, 1> cc = 1 / double((ba[1-1] + ba[2-1] + ba[3-1])) * (ba[1-1] * a + ba[2-1] * b + ba[3-1] * c);

    return myExpressionResultType(l, ba, cc);
}

void circumcenter(const Eigen::Vector3d& a, const Eigen::Vector3d& b, const Eigen::Vector3d& c, Eigen::Vector3d& cc)
{
    cc = myExpression(a, b, c).cc;
}
