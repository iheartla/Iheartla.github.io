...
struct myExpressionResultType {
    ...
    myExpressionResultType(const Eigen::MatrixXd & invLhs)
    : invLhs(invLhs)
    {}
};

myExpressionResultType myExpression(
    const Eigen::MatrixXd & TTT,
    const Eigen::MatrixXd & constraints)
{
    const long n = TTT.cols();
    const long m = constraints.rows();
    assert( TTT.rows() == n );
    assert( constraints.cols() == n );

    Eigen::MatrixXd invLhs_0(n+m, n+m);
    invLhs_0 << TTT, (constraints).transpose(),
    constraints, Eigen::MatrixXd::Zero(m, m);
    Eigen::MatrixXd invLhs = invLhs_0.inverse();

    return myExpressionResultType(invLhs);
}

bool Quadratic::constructInvLHS(const Eigen::MatrixXd & TTT)
{
  
  invLhs = myExpression(TTT, constraints).invLhs;
