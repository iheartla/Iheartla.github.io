...
myExpressionResultType myExpression(
    ...
    const Eigen::MatrixXd & poly2)
{
    const long n = poly.rows();
    assert( poly.cols() == 3 );
    assert( poly2.rows() == n );
    assert( poly2.cols() == 3 );
    ...
    for(int i=1; i<=d.size(); i++){
        ...
        H_0 << pow(d.at(i-1)[2-1], 2) + pow(d.at(i-1)[3-1], 2), -d.at(i-1)[1-1] * d.at(i-1)[2-1], -d.at(i-1)[1-1] * d.at(i-1)[3-1],
        ...
        sum_0 += 1 / double((d.at(i-1)).lpNorm<2>()) * H_0;
    }
    Eigen::Matrix<double, 3, 3> H = sum_0;

    Eigen::MatrixXd sum_1 = Eigen::MatrixXd::Zero(3, 1);
    for(int i=1; i<=d.size(); i++){
        Eigen::Matrix<double, 3, 1> J_0;
        J_0 << -d.at(i-1)[2-1] * p.at(i-1)[2-1] * q.at(i-1)[1-1] - d.at(i-1)[3-1] * p.at(i-1)[3-1] * q.at(i-1)[1-1] + d.at(i-1)[2-1] * p.at(i-1)[1-1] * q.at(i-1)[2-1] + d.at(i-1)[3-1] * p.at(i-1)[1-1] * q.at(i-1)[3-1],
        ...
        sum_1 += 1 / double((d.at(i-1)).lpNorm<2>()) * J_0;
    }
    Eigen::Matrix<double, 3, 1> J = sum_1;

    return myExpressionResultType(p, q, d, H, J);
}
...
Point SurfaceRemeshing::minimize_squared_areas(Vertex v)
{
    ...
    myExpressionResultType resultType = myExpression(poly, poly2);

    // compute minimizer
    Eigen::Vector3d x = resultType.H.lu().solve(-resultType.J);
