...
struct myExpressionResultType {
    ...
    myExpressionResultType(const Eigen::Matrix<double, 2, 2> & M,
               const Eigen::Matrix<double, 1, 6> & v)
    : M(M),
    v(v)
    {}
};
...
{
    Eigen::Matrix<double, 2, 2> M_0;
    M_0 << S_v[1-1], S_v[2-1],
    S_v[2-1], S_v[3-1];
    Eigen::Matrix<double, 2, 2> M_1;
    M_1 << cos(theta), -sin(theta),
    sin(theta), cos(theta);
    Eigen::Matrix<double, 2, 2> M = M_0 * M_1;

    Eigen::Matrix<double, 2, 1> v_0;
    v_0 << M(1-1, 1-1), M(2-1, 1-1);
    Eigen::Matrix<double, 2, 1> v_1;
    v_1 << M(1-1, 2-1), M(2-1, 2-1);
    Eigen::Matrix<double, 1, 6> v_2;
    v_2 << v_0.transpose() * TP, v_1.transpose() * TP;
    Eigen::Matrix<double, 1, 6> v = v_2;

    return myExpressionResultType(M, v);
}
