...
struct myExpressionResultType {
    Eigen::Matrix<double, 1, 3> p0;
    Eigen::Matrix<double, 1, 3> p1;
    Eigen::Matrix<double, 1, 3> p2;
    double x0s;
    Eigen::Matrix<double, 3, 3> Mi;
    Eigen::Matrix<double, 3, 1> m;
    myExpressionResultType(const Eigen::Matrix<double, 1, 3> & p0,
               const Eigen::Matrix<double, 1, 3> & p1,
               const Eigen::Matrix<double, 1, 3> & p2,
               const double & x0s,
               const Eigen::Matrix<double, 3, 3> & Mi,
    ...
    const Eigen::Matrix<double, 3, 1> & n)
{
    ...
    Eigen::Matrix<double, 1, 3> p0_0;
    p0_0 << V.at(v0)[1-1], V.at(v0)[2-1], V.at(v0)[3-1];
    Eigen::Matrix<double, 1, 3> p0 = p0_0;

    Eigen::Matrix<double, 1, 3> p1_0;
    p1_0 << V.at(v1)[1-1], V.at(v1)[2-1], V.at(v1)[3-1];
    Eigen::Matrix<double, 1, 3> p1 = p1_0;

    Eigen::Matrix<double, 1, 3> p2_0;
    p2_0 << V.at(v2)[1-1], V.at(v2)[2-1], V.at(v2)[3-1];
    Eigen::Matrix<double, 1, 3> p2 = p2_0;

    double x0s = pow((p0).norm(), 2);

    Eigen::Matrix<double, 3, 3> Mi_0;
    Mi_0 << 2 * (p1 - p0),
    2 * (p2 - p0),
    n.transpose();
    Eigen::Matrix<double, 3, 3> Mi = Mi_0.inverse();

    Eigen::Matrix<double, 3, 1> m_0;
    m_0 << pow((p1).norm(), 2) - x0s, pow((p2).norm(), 2) - x0s, (double)(p0 * n);
    Eigen::Matrix<double, 3, 1> m = Mi * m_0;

    return myExpressionResultType(p0, p1, p2, x0s, Mi, m);
}
            ...
            for (int i = 0; i < np; i++)
                for (int ii = 0; ii < P[i].tri.size(); ii++, ti++)
                    if (!ignore[ti])
                    {
                        ...
                        std::vector<Eigen::Matrix<double, 3, 1>> VP;
                        for (int i = 0; i < V.size(); ++i)
                        {
                            Eigen::Matrix<double, 3, 1> tmp_point;
                            tmp_point << V[i][0], V[i][1], V[i][2];
                            VP.push_back(tmp_point);
                        }
                        myExpressionResultType result_type = myExpression(v0, v1, v2, VP, n);
                        Eigen::RowVector3d p0 = result_type.p0;
                        Eigen::RowVector3d p1 = result_type.p1;
                        Eigen::RowVector3d p2 = result_type.p2;
                        double x0s = result_type.x0s;
                        Eigen::Matrix3d Mi = result_type.Mi;
                        Eigen::Vector3d m = result_type.m;
